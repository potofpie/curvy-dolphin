...
import { API_AGENT_ID, API_URL, APP_URL } from "../contstants";
...
const CameraCapture: React.FC<CameraCaptureProps> = ({
	...
	const checkForImage = useCallback(async (password: string) => {
		try {
	...
	// Polling effect for checking images when linkPassword is set
	useEffect(() => {
		if (!linkPassword) return;

		const interval = setInterval(() => {
			checkForImage(linkPassword);
		}, 2000); // Poll every 2 seconds

		return () => clearInterval(interval);
	}, [linkPassword, checkForImage]);
